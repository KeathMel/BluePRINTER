...
class CodeEditor(QPlainTextEdit):
    ...
    # Cyanotype palette — all of it from the shared theme
    PAPER      = QColor(theme.PAPER)         # the drawing surface
    GUTTER     = QColor(theme.INK)           # gutter sits deeper than the text
    GUTTER_TXT = QColor(theme.TEXT_DIM)      # muted line numbers
    ROW_LINE   = QColor(theme.LINE)          # faint separator between rows
    TEXT       = QColor(theme.TEXT)
    CUR_LINE   = QColor(theme.RAISED)        # current-line highlight
    ...
    def line_number_area_paint(self, event):
        ...
        painter.setPen(QColor(theme.LINE_STRONG))
