...
class ViewerWidget(QWidget):
    ...
    def __init__(self):
        ...
        self.viewer_label.setStyleSheet(f"background-color: {theme.PAPER};")

        layout = QVBoxLayout(self)
        layout.setContentsMargins(0, 0, 0, 0)
        layout.addWidget(self.viewer_label)
        self.setStyleSheet(f"background-color: {theme.PAPER};")
    ...
    def load_image(self, file_path):
        ...
        self.display_pixmap = pixmap.scaledToWidth(900, Qt.TransformationMode.SmoothTransformation)
    ...
    def refresh_display(self):
        ...
        for marker in self.markers:
            ...
            y = oy / self.scale_y
            r = int(15 * scale)
            painter.setPen(QPen(QColor(theme.MARKER), 2))
            mfill = QColor(theme.MARKER); mfill.setAlpha(140)
            painter.setBrush(QBrush(mfill))
            painter.drawEllipse(int(x - r), int(y - r), r * 2, r * 2)
            painter.setPen(QColor(theme.MARKER))
    ...
    def on_mouse_press(self, event):
        ...
        ox, oy = self.screen_to_image_coords(event.position().x(), event.position().y())
