...
from PyQt6.QtWidgets import QVBoxLayout, QHBoxLayout, QLabel, QSlider, QWidget, QSizePolicy
# Qt6 moved QOpenGLWidget out of QtWidgets into its own module
...
def default_gl_format():
    """The surface format this viewer needs.

    It draws with fixed-function OpenGL (glBegin/glEnd, gluSphere), which only
    exists in a DESKTOP GL compatibility context — not OpenGL ES. On Wayland,
    Qt6 goes through EGL and will hand back an ES context unless the renderable
    type and profile are stated explicitly, and EGL then rejects the mismatch
    with EGL_BAD_MATCH (0x3009). Naming all three is what avoids that.
    """
    fmt = QSurfaceFormat()
...
def _rgb(hex_colour):
    """'#rrggbb' -> the 0..1 floats OpenGL wants."""
    h = hex_colour.lstrip('#')
...
class Viewer3D(QWidget):
    ...
    def init_ui(self):
        ...
        scale_container.setStyleSheet(f"background-color: {theme.INK};")
...
class GL3DCanvas(QOpenGLWidget):
    ...
    def initializeGL(self):
        # the drawing surface matches the panels, so the model sits on
        # blueprint paper rather than a grey void
        _bg = _rgb(theme.PAPER)
        glClearColor(_bg[0], _bg[1], _bg[2], 1.0)
    ...
    def paintGL(self):
        ...
        _m = _rgb(theme.MARKER)
        glColor3f(_m[0], _m[1], _m[2])
    ...
    def _pos(self, event):
        """Mouse position as ints. Qt6 removed event.x()/y(); position()
        returns a QPointF, and the projection maths below wants numbers."""
        p = event.position()
        return int(p.x()), int(p.y())

    def mousePressEvent(self, event):
        ex, ey = self._pos(event)
        if event.button() == Qt.MouseButton.RightButton:
            ...
            hit = self.marker_at_screen(ex, ey)
            if hit is not None:
    ...
    def mouseMoveEvent(self, event):
        ...
        elif event.buttons() & Qt.MouseButton.LeftButton and self.dragging and self.selected_marker:
            # Unproject the cursor at the marker's captured screen depth so the
            # marker sticks exactly under the mouse.
            world = self.unproject_at_depth(ex, ey, self.drag_depth)
